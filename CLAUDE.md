...
## Current status (last updated 2026-06-19)
...
- ✅ **SEO complete.** Shell `<head>` has real title, meta description, canonical, robots,
  Open Graph, Twitter, JSON-LD (`ProfessionalService`), `lang="en"`, theme-color, and 3
  real favicon links. Template `<head>` also has title/description/canonical for Google's
  JS render. Verified valid + no duplicate tags.
...
(only if the export lacks it), 3 real favicon links in the shell, a readable `<noscript>`
fallback, gtag (`G-QZ5SEZVZBH`), and title/description/canonical in the template head.
A `<!-- seo-shell:applied -->` marker + per-feature checks make re-runs safe (zero changes
on re-run). Edit the CONFIG block at the top if brand copy / GA id / canonical URL change.
